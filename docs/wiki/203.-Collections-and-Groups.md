...
err := fm.Components().ForEach(func(ctx context.Context, c *component.Component) error {
...
err := port.Signals().ForEach(func(s *signal.Signal) error {
...
doubledInts := port.Signals().MapIf(
    func(s *signal.Signal) bool { _, ok := s.Payload().(int); return ok },
...
_ = c.Inputs().PutSignalsOnEach(signal.New(1))

// Flush all output ports
_ = c.Outputs().Flush()

// Pipe all outputs to the same destination
_ = c.Outputs().PipeEachTo(destPort)
...
_ = fm.Components().
    ...
    ForEach(func(ctx context.Context, c *component.Component) error {
...
```go
fm.Components().ForEach(func(ctx context.Context, c *component.Component) error { ... })
