...
`Sequential` runs the functions in order and stops at the first error. The error is the component's own, not a hook failure, so a stage that returns `component.ErrWaitingForInputsKeep` suspends the component exactly as it would have if the whole activation were written as one function.
...
| `component.RequireInputs(names...)` | Returns `ErrWaitingForInputsKeep` — the component is suspended and **keeps** what already arrived | The component **needs** every named input before it can compute anything |
