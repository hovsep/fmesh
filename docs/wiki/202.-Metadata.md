...
| `component`, `port`, `fmesh`, collections | Mutating — go through the store | `Labels().Set(k,v)` | `Labels().SetMany(map)` | `Labels().Clear().SetMany(map)` | `Labels().Remove(...)` |

Signals and signal groups are **immutable** — `With*` methods return a new value, and they are the
only way to get a modified signal. Components, ports and meshes are **mutable**, and carry no
metadata methods of their own: `Labels()` and `Scalars()` hand you the live store and you change
that. Chaining still works, on the store:

```go
c.Labels().Set("layer", "business-logic").Set("owner", "team-a")
```

The split means `With*` has exactly one meaning across the whole API — it returns something new.
...
// Or chain Set calls
c.Labels().Set("layer", "business-logic").
    Set("category", "data-processing").
    Set("criticality", "high")

// Add labels after creation
c.Labels().Set("version", "2.0").Set("owner", "team-a")
```
...
c.InputByName("data").Labels().
    Set("type", "primary").
    Set("required", "true")

c.InputByName("config").Labels().
    Set("type", "configuration").
    Set("optional", "true")

c.OutputByName("logs").Labels().
    Set("type", "diagnostic")

// Or replace the whole set at once
c.InputByName("data").Labels().Clear().SetMany(map[string]string{
    "type":     "primary",
    "required": "true",
})
```
...
_, _ = component.New("router",
    ...
    component.WithActivationFunc(func(ctx context.Context, c *component.Component) error {
        err := c.InputByName("in").Signals().ForEach(func(sig *signal.Signal) error {
            priority := sig.Labels().ValueOrDefault("priority", "normal")
            switch priority {
            case "high":
