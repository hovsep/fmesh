...
`Run()` returns `(*RuntimeInfo, error)`, populated even when the run fails. Reading it — the fields,
the per-cycle walk, and what each activation result carries — is covered on
[402. Inspecting a run](https://github.com/hovsep/fmesh/wiki/402.-Inspecting-a-run); this page only
covers the internals underneath it.
...
| `Labels()`, `Scalars()` | Cycle-level metadata stores (mutate in place) |
...
`component.MaybeActivate(ctx)` is called by the scheduler for each component each cycle. It can be used in tests to activate a single component outside a full mesh run. Returns `*ActivationResult`.
