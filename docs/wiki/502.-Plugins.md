...
fm, err := fmesh.New("mesh",
    fmesh.WithPlugins(plugin.Broadcast("time")),
)
...
| Constructor | Rule |
|-------------|------|
| `plugin.Broadcast("time")` | every output named `time` → every input named `time` |
| `plugin.BroadcastAs("tick", "time")` | every output named `tick` → every input named `time` |
| `plugin.Prefixed("env_")` | an output → the input named `<prefix><component>_<port>`, e.g. `env_sun_uvi` |
